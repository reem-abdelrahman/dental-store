...
import { dental_product, Product } from '../../models/store';
...
describe("test CRUD methods", ()=>{
    ...
    let result: order
    const dummy_order: order = {user_id: "", status: "active"}
    ...
    beforeAll( async()=>{
       P = await create_product.create({
        ...
        if(new_user.id) dummy_order.user_id = new_user.id.toString()
    })    
    ...
    it("should add a product to the order", async ()=>{
        const added_product: added_prod = await shop.addProd({
            quantity: 15,
