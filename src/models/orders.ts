...
export type order = {
    ...
    quantity?: added_prod["quantity"];
    order_id?: added_prod["order_id"];
...
export class Order {
   ...
   async show_order_status(status: string): Promise<order[]> {
    ...
    async addProd(p: added_prod): Promise<added_prod> {
        try{
            const sql: string = 'INSERT INTO product_order (quantity, order_id, product_id) VALUES($1, $2, $3) RETURNING *';
      const conn: PoolClient = await database.connect()
      const result: QueryResult = await conn
