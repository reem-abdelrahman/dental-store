...
const salt: string = process.env
  .SALT_ROUNDS as string;
const pepper: string = process.env
  .BCRYPT_PASSWORD as string;
export class User {
  async view_users(): Promise<user[]> {
    try {
      ...
      throw new Error(
        `Error: ${err} Error with showing all users`
      );
    }
  }
  ...
  async show_user(id: number): Promise<user> {
    try {
      ...
      throw new Error(
        ` Error: ${err}. Could not find the user with the id: ${id}.`
      );
    }
  }
  ...
  async create(u: created_user): Promise<user> {
    try {
      ...
      throw new Error(
        `Error: ${err}. Could not create the user ${u}. `
      );
    }
  }
