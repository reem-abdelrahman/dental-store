...
export class Product {
    async view_all(): Promise<dental_product[]> {
        ...
        const sql: string = 'SELECT * from dental_products';
    ...
    async show_product_id(id: number): Promise<dental_product> {
      try {
      const sql:string = 'SELECT * FROM dental_products WHERE id=($1)';
    ...
    async show_product_category(categ: string): Promise<dental_product> {
      ...
      const conn: PoolClient = await database.connect()
      const result: QueryResult = await conn.query(sql, [categ])
      conn.release()
      return result.rows[0]
      } catch (err) {
          throw new Error(` Error: ${err}. Could not find the product with the category: ${categ}.`)
      }
    }

    async create(p: product): Promise<dental_product> {
        try {
      const sql: string = 'INSERT INTO dental_products (name, price, category) VALUES($1, $2, $3) RETURNING *';
      const conn: PoolClient = await database.connect()
